...
export default function LoginPage() {
  ...
  const [isAutoLoggingIn, setIsAutoLoggingIn] = useState(true); // เพิ่ม state นี้
  const [hasRedirected, setHasRedirected] = useState(false); // เพิ่ม flag นี้
  const goByRole = useCallback(
    (role: Role) => {
      if (hasRedirected) return; // ป้องกัน redirect ซ้ำ
      
      console.log(`[Navigation] Redirecting to: /${role}`);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        console.log("[Auto-Login] Found stored data:", parsed);
        
        // ตรวจสอบ format ข้อมูล
        const accessToken = parsed.tokens?.access_token || parsed.access_token;
        
        if (!accessToken) {
          console.warn("[Auto-Login] No access token found");
          localStorage.removeItem(STORAGE_KEY);
          setIsAutoLoggingIn(false);
          return;
        }
        console.log("[Auto-Login] Checking token validity...");
        const profileRes = await fetch(`${DEFAULT_BASE_URL}/auth/profile`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!profileRes.ok) {
          console.warn("[Auto-Login] Token expired or invalid.");
          localStorage.removeItem(STORAGE_KEY);
          setIsAutoLoggingIn(false);
          return;
        }
        const me = (await profileRes.json()) as any;
        console.log("[Auto-Login] Success! Profile loaded:", me);
        
        // Update storage with fresh data
        const storageData = { tokens: { access_token: accessToken }, ...me };
    ...
    void run();
  }, []); // ลบ goByRole dependency เพื่อป้องกัน infinite loop
  // แสดง loading screen ระหว่าง auto-login
  if (isAutoLoggingIn) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-[#0f081f] via-[#140f2e] to-[#0a0518] text-white flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
          <p className="text-gray-300">กำลังตรวจสอบการเข้าสู่ระบบ...</p>
        </div>
      </main>
    );
  }
  const handleSignIn = async (event: FormEvent) => {
    ...
    try {
      ...
      console.log("[2/3] Signin Success! Tokens received.");
      ...
      const me = (await profileRes.json()) as any;
      console.log("[3/3] Profile Loaded Successfully:", me);
      // บันทึกข้อมูลใน format ที่ถูกต้อง
      const storageData = { tokens, ...me };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(storageData));
